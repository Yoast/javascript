...
describe( "Tests for the ranking intention assessment for English", function() {
	...
	it( "runs the ranking intention on the paper with keyword", function() {
		...
		expect( result.getText() ).toBe( "<a href='https://yoa.st/33v' target='_blank'>Ranking intention</a>: Your text does not reflect any particular ranking intention. If your keywords is singular, use more singular occurrences; if your keyphrase is plural, use more plural occurrences!" );
	} );
	it( "runs the ranking intention on the paper with keyword", function() {
		...
		expect( result.getText() ).toBe( "<a href='https://yoa.st/33v' target='_blank'>Ranking intention</a>: Include your keyphrase in the text so that we can check ranking intention." );
	} );
} );
...
describe( "Checks if the assessment is applicable", function() {
	...
	it( "is applicable when there is a text of 100+ words and the keyphrase is set", function() {
		const mockPaper = new Paper( "word ".repeat( 100 ) + "fluffy cat", { keyword: "fluffy cat" } );
	...
	it( "is not applicable when there is no text", function() {
		const mockPaper = new Paper( "", { keyword: "fluffy cat" } );
		const isApplicable = SingularPluralAssessment.isApplicable( mockPaper );
		expect( isApplicable ).toBe( false );
	} );

	it( "is not applicable when the text is shorter than 100 words", function() {
		const mockPaper = new Paper( "word ".repeat( 50 ) + "fluffy cat", { keyword: "fluffy cat" } );
		const isApplicable = SingularPluralAssessment.isApplicable( mockPaper );
		expect( isApplicable ).toBe( false );
	} );
